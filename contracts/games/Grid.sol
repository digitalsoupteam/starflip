...
contract Grid is VRFConsumerBaseV2Plus, UUPSUpgradeable, IGame {
    ...
    /**
     * @notice Sets the winning and target cells quantities (owners multisig only)
     * @dev Allows the owners multisig to update both the number of winning cells generated by the contract
     *      and the number of cells a player can select in a single transaction
     * @param newWinCellsQuantity The new winning cells quantity
     * @param newTargetCellsQuantity The new target cells quantity
     */
    function setCellsQuantities(uint8 newWinCellsQuantity, uint8 newTargetCellsQuantity) external {
        addressBook.accessRoles().requireOwnersMultisig(msg.sender);

        require(newWinCellsQuantity > 0, "Win cells quantity must be greater than 0");
        require(newWinCellsQuantity <= totalCellsQuantity, "Win cells quantity must be less than or equal to total cells");

        require(newTargetCellsQuantity > 0, "Target cells quantity must be greater than 0");
        require(newTargetCellsQuantity <= totalCellsQuantity, "Target cells quantity must be less than or equal to total cells");
